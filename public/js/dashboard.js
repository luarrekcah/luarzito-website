...
window.onload = () => {
  ...
  if(localStorage.getItem("login") == "" && localStorage.getItem == undefined) {
    console.log("vazio")
  } else {
    
      //SETANDO VARIAVEIS UNDEFINED
    const req = localStorage.getItem("login");
      perfil = JSON.parse(req);
  ...
  if (location.href.indexOf("code") > -1) {
    ...
    req.onload = () => {
      ...
      console.log(req);
      localStorage.setItem("login", req.responseText);
